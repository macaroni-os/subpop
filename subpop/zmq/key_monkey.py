...
#	stored in:
#
#	~/.curve/authorized_clients.server/clientname.key


class KeyMonkeyError(Exception):
	pass
...
class KeyMonkey(object):
	...
	def __init__(self,myid="id_curve"):
		...
		self.authorized_clients_dir = self.curvedir + "/authorized_clients.%s" % self.myid
	...
	def setupServer(self, server, endpoint):
		try:
			...
			raise KeyMonkeyError("Couldn't load private key: %s" % self.private_key)
